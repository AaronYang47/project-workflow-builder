...
export function getAdaptiveNodeSize(node: DomainNode, current?: Pick<NodeLayout, "width" | "height">) {
  ...
  if (isReferenceNodeType(node.type)) {
    ...
    const lineCount = (items: string[] | undefined) => Math.max(1, (items || []).reduce((total, item) => total + Math.max(1, Math.ceil(item.length / 26)), 0));
    let width = fallback.width;
    let height = fallback.height;
    if (node.type === "approvalMatrix") height = Math.max(height, 100 + (reference.rows?.length || 0) * 38);
    if (node.type === "controlBackbone" || node.type === "responsibilityLane") {
      width = Math.max(width, Math.min(6, Math.max(1, reference.sections?.length || 1)) * 255 + 32);
      height = Math.max(height, 126 + Math.max(0, ...(reference.sections || []).map((section) => lineCount(section.items) * 17)));
    }
    if (node.type === "serviceLegend") height = Math.max(height, 90 + Math.ceil((reference.items?.length || 0) / 2) * 48);
    if (node.type === "jobNumbering") height = Math.max(height, 116 + Math.max(lineCount(reference.current), lineCount(reference.proposed)) * 17);
    if (node.type === "businessRules") height = Math.max(height, 90 + lineCount(reference.rules) * 24);
    if (node.type === "terminal") height = Math.max(height, 100 + Math.ceil(node.title.length / 36) * 24 + Math.ceil(node.description.length / 60) * 17);
    return { width: Math.max(current?.width || 0, width), height: Math.max(current?.height || 0, height) };
  }
  ...
  const minWidth = generalCard ? 320 : terminal ? 210 : 240;
  const width = clamp(Math.max(minWidth, 142 + node.title.length * 5, 80 + longestWord * 7), minWidth, generalCard ? 480 : 380);
